...
attach to and watch live. Like `tmux`, but built for agents, with **8 MCP
tools**, a universal one-command installer, and a full TUI for you.
...
`interactive-terminal-mcp` on your `PATH`, then opens an interactive setup: it
detects your AI clients, lets you toggle which ones to wire up with the arrow
keys, and asks how long to keep session logs. Run
`interactive-terminal-mcp configure` anytime to change it.
...
- **Real terminal emulation.** Full VT support with an alternate screen,
  scrollback, and DEC modes, so `vim`, `less`, `htop`, and `tmux` behave the way
  they do in your own terminal — not a mangled approximation.
- **Keystrokes, not just commands.** `"CTRL+C"`, `"ESC; :wq; ENTER"`,
  `"DOWN*5"`. Arrow keys are encoded for whatever the running program expects,
  which is why they work *inside* `vim` rather than only at a prompt.
...
| Tool | What it does |
|---|---|
| `it_active` | Report or switch the active session, and return its screen |
| `it_list` | List sessions, newest activity first, token-budget paginated |
| `it_new` | Create a session, make it active, return its first screen |
| `it_read` | Return a session's current screen |
| `it_send` | Type text and/or keystrokes, then return the screen |
| `it_kill` | End a session (always requires an explicit session) |
...
Nothing is ever silently truncated: the count of omitted lines and an absolute
path to the complete file always come back with the answer.

## Install

**macOS / Linux** — downloads to `~/.interactive-terminal-mcp/bin` and adds it
to your `PATH`:
...
**Open a new terminal** afterward so the command is found.
...
Detects your installed clients, pre-checks the ones already set up, and lets you
toggle with the arrow keys. It writes the absolute path to the binary plus the
`mcp` subcommand into each client's config, so nothing depends on that client's
working directory or `PATH`. Re-running is idempotent and leaves unrelated
settings, comments, and formatting alone.

Registration is handled by
[detect-harness](https://github.com/sairaph/detect-harness), which distinguishes
*not installed* from *could not inspect* and never overwrites a same-name entry
that belongs to something else.

### Session logs

Setup asks one question:
...
sessions are never affected. Change it later in
`interactive-terminal-mcp configure`.

## The interactive app

Run `interactive-terminal-mcp` with no arguments on a terminal:
...
- **Automatic raw mode.** The moment a program takes the alternate screen, every
  keystroke goes straight to it — `vim` and `htop` just work. `ctrl+g` toggles
  manually for programs that read keys without using the alternate screen.
...
`c` from the list opens the same settings screen the installer shows.
...
interactive-terminal-mcp new [name] [--cmd C]      create a session
interactive-terminal-mcp attach <session>          open it in this terminal
interactive-terminal-mcp read [session]            print its screen
interactive-terminal-mcp send [session] --text T   type into it
interactive-terminal-mcp send [session] --keys K   send keystrokes
interactive-terminal-mcp tail [session] [-n N]     recent output
interactive-terminal-mcp head [session] [-n N]     earliest output
interactive-terminal-mcp kill <session>            end it
interactive-terminal-mcp status                    daemon status
interactive-terminal-mcp doctor                    diagnose the install
```
...
The socket is mode `0600` in a `0700` directory (a per-user named pipe with an
...
```bash
git clone https://github.com/sairaph/interactive-terminal-mcp
